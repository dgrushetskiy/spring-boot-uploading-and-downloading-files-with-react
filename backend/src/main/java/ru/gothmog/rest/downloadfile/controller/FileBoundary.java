package ru.gothmog.rest.downloadfile.controller;
...
import ru.gothmog.rest.downloadfile.exceptions.ResourceNotFoundException;
import ru.gothmog.rest.downloadfile.model.FileEntity;
import ru.gothmog.rest.downloadfile.repository.FileEntityRepository;
...
public class FileBoundary {
    ...
    public ResponseEntity<byte[]> getRandomFile() {
        ...
        header.setContentLength(fileEntity.getImage().length);
        header.set("Content-Disposition", "attachment; filename=" + fileEntity.getFileName());

        return new ResponseEntity<>(fileEntity.getImage(), header, HttpStatus.OK);
    }
